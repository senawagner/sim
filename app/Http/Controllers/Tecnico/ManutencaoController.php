...
class ManutencaoController extends Controller
{
    public function index()
    {
        $manutencoes = Manutencao::with(['filial', 'equipamento', 'usuario'])
            ...
            ->paginate(10);
        return view('tecnico.manutencoes.index', compact('manutencoes'));
    }
    ...
    public function create()
    {
        ...
        return view('tecnico.manutencoes.create', compact('filiais', 'equipamentos'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('tecnico.manutencoes.index')->with('success', 'Manutenção registrada com sucesso!');
        } catch (\Exception $e) {
            Log::error('Erro ao registrar manutenção', ['error' => $e->getMessage()]);
            return redirect()->route('tecnico.manutencoes.create')->with('error', 'Erro ao registrar manutenção. Por favor, tente novamente.');
        }
    }

    public function show(Manutencao $manutencao)
    {
        return view('tecnico.manutencoes.show', compact('manutencao'));
    }
    ...
    public function edit(Manutencao $manutencao)
    {
        $filiais = Filial::all();
        $equipamentos = Equipamento::all();
        return view('tecnico.manutencoes.edit', compact('manutencao', 'filiais', 'equipamentos'));
    }
    ...
    public function update(Request $request, Manutencao $manutencao)
    {
        ...
        try {
            ...
            return redirect()->route('tecnico.manutencoes.index')->with('success', 'Manutenção atualizada com sucesso!');
        } catch (\Exception $e) {
            Log::error('Erro ao atualizar manutenção', ['error' => $e->getMessage(), 'manutencao_id' => $manutencao->id]);
            return redirect()->route('tecnico.manutencoes.edit', $manutencao)->with('error', 'Erro ao atualizar manutenção. Por favor, tente novamente.');
        }
    }
    ...
    public function destroy(Manutencao $manutencao)
    {
        try {
            ...
            return redirect()->route('tecnico.manutencoes.index')->with('success', 'Manutenção excluída com sucesso!');
        } catch (\Exception $e) {
            Log::error('Erro ao excluir manutenção', ['error' => $e->getMessage(), 'manutencao_id' => $manutencao->id]);
            return redirect()->route('tecnico.manutencoes.index')->with('error', 'Erro ao excluir manutenção. Por favor, tente novamente.');
        }
    }
